...
function loadStatsPage()
{
	...
	var statsTable = document.getElementById("stats_table"); //Retrieve our table element
	var colWin = document.getElementById("wins");
	var colLoss = document.getElementById("losses");

	for(row=2; row < statsTable.rows.length; row++)
	{
		if(parseInt(statsTable.rows[row].cells[2].innerHTML) > parseInt(statsTable.rows[row].cells[3].innerHTML))
		{
			statsTable.rows[row].cells[4].innerHTML = 'Cu Boulder';
			wins++;
		}
		else
		{
			statsTable.rows[row].cells[4].innerHTML = statsTable.rows[row].cells[1].innerHTML;
			loss++;
		}
	}
	colWin.innerHTML = wins;
	colLoss.innerHTML = loss;
}
...
function switchPlayers(playerNum)
{
	...
	document.getElementById('p_year').innerHTML = players[playerNum].name;
	...
	document.getElementById('player_img').innerHTML = players[playerNum].fruit_color;
